...
def act_quant(x: torch.Tensor,
    ...
    assert x.size(
        -1
    ) % block_size == 0, f'Last dimension size must be divisible by block_size (block_size={block_size})'
...
fp8_gemm_configs = [
    Config(
        ...
        num_warps=8) for block_m in [16, 32, 64] for block_n in [32, 64, 128]
...
@triton.jit
def fp8_gemm_kernel(a_ptr, b_ptr, c_ptr, a_s_ptr, b_s_ptr, M, N: tl.constexpr,
