...
class ParallelEmbedding(nn.Module):
    ...
    def __init__(self, vocab_size: int, dim: int):
        ...
        assert vocab_size % world_size == 0, (
            "Vocabulary size must be divisible by world size "
    ...
    def __init__(self,
        ...
        if self.weight.element_size() == 1:
            ...
            self.weight.scale = self.scale = nn.Parameter(
                torch.empty(scale_out_features,
                            scale_in_features,
                            dtype=torch.float32))
        else:
...
class ColumnParallelLinear(Linear):
    ...
    def __init__(self,
        ...
        assert out_features % world_size == 0, (
            "Output features must be divisible by "
            f"world size (world_size={world_size})")
...
class RowParallelLinear(Linear):
    ...
    def __init__(self,
        ...
        assert in_features % world_size == 0, (
            "Input features must be divisible by "
            f"world size (world_size={world_size})")
...
class MLA(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        if self.q_lora_rank == 0:
            self.wq = ColumnParallelLinear(self.dim,
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor,
        ...
        if attn_impl == "naive":
            ...
            scores = (torch.einsum("bshc,btc->bsht", q_nope,
                      ...
                      torch.einsum(
                          "bshr,btr->bsht", q_pe,
                          self.pe_cache[:bsz, :end_pos])) * self.softmax_scale
        if mask is not None:
...
class Gate(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        self.bias = nn.Parameter(torch.empty(
            args.n_routed_experts)) if self.dim == 7168 else None
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.n_groups > 1:
            ...
            scores = scores.masked_fill_(mask.unsqueeze(-1),
                                         float("-inf")).flatten(1)
...
class Block(nn.Module):
    ...
    def __init__(self, layer_id: int, args: ModelArgs):
        ...
        self.ffn = MLP(
            args.dim,
            args.inter_dim,
        ) if layer_id < args.n_dense_layers else MoE(args)
...
class Transformer(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        Linear.dtype = (torch.float8_e4m3fn
                        if args.dtype == "fp8" else torch.bfloat16)
        ...
        self.head = ColumnParallelLinear(args.dim,
                                         args.vocab_size,
    ...
    def forward(self, tokens: torch.Tensor, start_pos: int = 0):
        ...
        mask = None
        if seqlen > 1:
            mask = torch.full((seqlen, seqlen),
                              float("-inf"),
                              device=tokens.device).triu_(1)
        for layer in self.layers:
            h = layer(h, start_pos, freqs_cis, mask)  # transformer block
        h = self.norm(h)[:, -1]
        logits = self.head(h)
        if world_size > 1:
